...
@router.get("", summary="Global keyword & semantic search", description="Perform hybrid SQL keyword and vector semantic search across contracts and policies")
def search(
    q: str = Query("", alias="q"),
    ...
    if search_type in ['all', 'policies']:
        try:
            ...
            for match in semantic_matches:
                ...
                policy = db.query(Policy).filter(Policy.id == p_id).first() if p_id else None
                if policy:
