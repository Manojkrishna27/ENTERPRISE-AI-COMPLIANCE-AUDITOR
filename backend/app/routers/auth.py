...
@router.get(
    "/departments",
    summary="List departments for signup",
    description="Fetch public list of available departments for user registration",
)
...
@router.post(
    ...
    description="Register a new user in the compliance system",
)
def register(data: UserRegisterSchema, request: Request, db: Session = Depends(get_db)):
    ...
    log_audit(
        user.id,
        "USER_REGISTER",
        f"Registered new account: {user.email} (auto_verified={auto_verify})",
        ip_address=client_ip,
    )

    msg = (
        "User registered successfully! Account is ready for login."
        if auto_verify
        else "User registered successfully. Please verify your email."
    )
    return {"msg": msg, "user": user.to_dict()}


@router.post(
    "/login",
    summary="Login and get JWT tokens",
    description="Authenticate user credentials and return access JWT token",
)
...
@router.post(
    "/logout",
    summary="Logout user",
    description="Revoke current access token and add to Redis blocklist",
)
...
@router.post(
    ...
    description="Generate password reset token for account recovery",
)
def forgot_password(
    data: ForgotPasswordSchema, request: Request, db: Session = Depends(get_db)
):
    user = db.query(User).filter(User.email == data.email).first()
    if user:
...
def reset_password(
    data: ResetPasswordSchema, request: Request, db: Session = Depends(get_db)
):
    ...
    client_ip = request.client.host if request.client else "System"
    log_audit(
        user.id, "PASSWORD_RESET", "Reset password via token", ip_address=client_ip
    )
...
def verify_email(
    data: VerifyEmailSchema, request: Request, db: Session = Depends(get_db)
):
    if not data.token:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
