...
def process_and_index_contract(version_id: str, local_path: str, file_ext: str):
    ...
    try:
        version = (
            db.query(ContractVersion).filter(ContractVersion.id == version_id).first()
        )
        if not version:
            return
...
@router.get(
    ...
    description="Fetch list of contracts filtered by department or status",
)
def list_contracts(
...
@router.post(
    ...
    description="Upload contract file (PDF/DOCX) and store headers and version details",
)
async def upload_contract(
    ...
    if not name:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Contract name is required"
        )
    ...
    try:
        ...
        log_audit(
            current_user.id,
            "CONTRACT_UPLOAD",
            f"Uploaded contract: {name} (Version 1)",
...
@router.get(
    ...
    description="Fetch contract by ID with all versions",
)
def get_contract(
    id: str,
    current_user: User = Depends(get_current_user),
    ...
    versions = (
        ...
        .filter(ContractVersion.contract_id == id)
...
@router.get(
    "/{id}/versions/{ver_id}",
    summary="Get contract version details",
    description="Fetch contract version and parsed text chunks",
)
def get_version(
    id: str,
    ver_id: str,
    ...
    version = (
        db.query(ContractVersion)
        .filter(ContractVersion.id == ver_id, ContractVersion.contract_id == id)
        .first()
    )
    if not version:
        raise HTTPException(
...
@router.post(
    ...
    description="Upload new version for an existing contract",
)
async def upload_version(
    ...
    try:
        ...
        db_chunks = (
            db.query(ContractChunk).filter(ContractChunk.version_id == version.id).all()
        )
        qdrant_service.index_contract_chunks(version.id, db_chunks)
        db.commit()
        ...
        log_audit(
            current_user.id,
            "CONTRACT_VERSION_UPLOAD",
            f"Uploaded version {new_version_num} for contract: {contract.name}",
            ip_address=client_ip,
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to upload version: {e!s}",
        )
...
@router.delete(
    ...
    description="Delete contract and all associated versions & vector embeddings",
)
def delete_contract(
    id: str,
    request: Request,
    ...
    log_audit(
        current_user.id,
        "CONTRACT_DELETE",
        f"Deleted contract: {contract.name}",
        ip_address=client_ip,
    )
...
@router.post(
    ...
    description="Change contract status to Archived",
)
def archive_contract(
    ...
    log_audit(
        current_user.id,
        "CONTRACT_ARCHIVE",
        f"Archived contract: {contract.name}",
        ip_address=client_ip,
    )
...
@router.post(
    ...
    description="Restore archived contract status back to Draft",
)
def restore_contract(
    ...
    log_audit(
        current_user.id,
        "CONTRACT_RESTORE",
        f"Restored contract: {contract.name}",
        ip_address=client_ip,
    )
