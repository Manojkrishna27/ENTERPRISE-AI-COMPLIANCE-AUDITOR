...
@router.get(
    ...
    description="Fetch list of audit reports filtered by contract ID",
)
def list_reports(
...
@router.post(
    "/contracts/{contract_id}/version/{ver_id}/generate",
    status_code=status.HTTP_201_CREATED,
    summary="Generate compliance PDF report",
    description="Generate executive PDF compliance audit report for contract version",
)
def generate_report(
    contract_id: str,
    ver_id: str,
    ...
    if not contract or not version:
        raise HTTPException(
            ...
            detail="Contract or version not found",
        )
    ...
    table_data = [
        [
            Paragraph("<b>Risk Metric</b>", bold_body_style),
            Paragraph("<b>Count</b>", bold_body_style),
            Paragraph("<b>System Severity</b>", bold_body_style),
        ],
        [
            Paragraph("High Risk Violations", body_style),
            str(high_count),
            "Requires Immediate Amendment",
        ],
        [
            Paragraph("Medium Risk Violations", body_style),
            str(med_count),
            "Recommended Remediation",
        ],
        [
            Paragraph("Low Risk Anomalies", body_style),
            str(low_count),
            "Minor Adjustment",
        ],
    ]
    ...
    if not findings:
        story.append(
            Paragraph(
                "No compliance breaches or policy deviations detected in this version.",
                body_style,
            )
        )
    else:
        for idx, f in enumerate(findings):
            ...
            story.append(
                Paragraph(
                    f"<b>Category:</b> {f.category}  |  <b>Risk Level:</b> {f.risk_level}  |  <b>Confidence Score:</b> {conf_score}%",
                    body_style,
                )
            )
            ...
            if f.business_impact:
                story.append(
                    Paragraph(
                        f"<b>Business Impact:</b> {f.business_impact}", body_style
                    )
                )
            if f.recommendation:
                story.append(
                    Paragraph(
                        f"<b>Remediation Action:</b> {f.recommendation}", body_style
                    )
                )
    ...
    try:
        ...
        log_audit(
            current_user.id,
            "REPORT_GENERATE",
            f"Generated compliance report for contract: {contract.name}",
            ip_address=client_ip,
        )

        return {"msg": "Report generated successfully", "report": report.to_dict()}
...
@router.get(
    ...
    description="Download generated compliance PDF report binary file",
)
def download_report(
    id: str,
    current_user: User = Depends(get_current_user),
    ...
    try:
        ...
        return FileResponse(
            path=local_path, media_type="application/pdf", filename=download_name
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve report binary file: {e!s}",
        )
