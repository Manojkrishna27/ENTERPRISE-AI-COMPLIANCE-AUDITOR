...
from app.schemas.auth import DepartmentCreateSchema, UserRoleUpdateSchema
...
@router.get(
    ...
    description="Fetch list of all registered users across departments",
)
def list_users(
    current_user: User = Depends(role_required("Admin")), db: Session = Depends(get_db)
):
...
@router.put(
    ...
    description="Modify role permission assignment for a user",
)
def update_user_role(
    ...
    if not new_role or new_role not in [
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid or missing role"
        )
    ...
    log_audit(
        current_user.id,
        "USER_ROLE_UPDATE",
        f"Updated user {target_user.email} role from {old_role} to {new_role}",
...
@router.get(
    ...
    description="Fetch list of all corporate departments",
)
...
@router.post(
    ...
    description="Add a new organization department",
)
...
@router.get(
    ...
    description="Fetch recent security and system audit log events",
)
def list_audit_logs(
