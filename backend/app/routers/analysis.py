...
@router.post("/contracts/{contract_id}/version/{ver_id}/analyze", summary="Run compliance analysis", description="Perform AI RAG analysis against compliance guidelines")
def run_analysis(
    contract_id: str,
    ver_id: str,
    request: Request,
    ...
    if not chunks:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No text chunks found for this contract version. Verify if parsed correctly.")
    ...
    try:
        ...
        matching_policies = qdrant_service.search_policy_chunks(compliance_query, limit=10, department_id=contract.department_id if contract else None)
        ...
        log_audit(current_user.id, "CONTRACT_ANALYZE", f"Analyzed contract: {contract.name if contract else ''} (V{version.version_number})", ip_address=client_ip)
        ...
        db.commit()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed during compliance analysis run: {str(e)}")


@router.get("/contracts/{contract_id}/version/{ver_id}/findings", summary="Get contract findings", description="Fetch list of AI findings for contract version")
def get_findings(
    contract_id: str,
    ver_id: str,
    ...
    findings = db.query(AIFinding).filter(AIFinding.version_id == ver_id).order_by(AIFinding.risk_level.desc()).all()
    return [f.to_dict() for f in findings]


@router.post("/contracts/{contract_id}/version/{ver_id}/copilot", summary="Copilot RAG Chat", description="Query AI Copilot with contract & policy context retrieval")
def copilot_chat(
    contract_id: str,
    ver_id: str,
    ...
    chunks = db.query(ContractChunk).filter(ContractChunk.version_id == ver_id).order_by(ContractChunk.chunk_position.asc()).all()
    if not chunks:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No content found for this contract version")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"RAG retrieval or LLM generation failed: {str(e)}")
