...
@router.get("", summary="Get dashboard metrics & analytics", description="Fetch KPI metrics, risk distributions, upload stats, and recent activity logs")
def get_analytics(
    ...
    pending_reviews = db.query(Contract).filter(Contract.status == 'Pending Review').count()
