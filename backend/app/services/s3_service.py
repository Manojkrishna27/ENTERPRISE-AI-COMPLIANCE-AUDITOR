...
class S3Service:
    def __init__(self):
        ...
        self.provider = os.environ.get('STORAGE_PROVIDER', 'aws_s3' if not self.use_local else 'local').lower()
        ...
        if not self.use_local:
            try:
                ...
                print(f"Could not initialize S3/MinIO client ({e}). Defaulting to local storage.")
                self.use_local = True
