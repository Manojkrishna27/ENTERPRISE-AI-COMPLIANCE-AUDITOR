...
class QdrantService:
    ...
    def _ensure_collection_exists(self, collection_name):
        ...
        try:
            ...
            if collection_name not in collection_names:
                ...
                print(f"Created Qdrant collection: {collection_name} with dim {ai_config.embedding_dimension}")
        except Exception as e:
            print(f"Error checking/creating Qdrant collection {collection_name}: {e}. Falling back to memory.")
            self.client = None
    ...
    def index_policy_chunks(self, policy_id, chunks):
        ...
        for chunk in chunks:
            ...
            if self.client:
                points.append(qmodels.PointStruct(id=qdrant_uuid, vector=embedding, payload=payload))
            else:
                self.fallback_db[qdrant_uuid] = {"vector": embedding, "payload": payload}
    ...
    def index_contract_chunks(self, version_id, chunks):
        ...
        if self.client and points:
            try:
                self.client.upsert(collection_name=self.contract_collection, points=points)
                print(f"Indexed {len(points)} chunks in Qdrant for contract version {version_id}")
            except Exception as e:
                print(f"Failed upsert to Qdrant: {e}")
    ...
    def search_policy_chunks(self, query_text, limit=5, department_id=None):
        ...
        if self.client:
            try:
                ...
                formatted = [dict(res.payload, score=res.score, id=res.id) for res in results]
                latency = time.time() - start_time
                self._log_retrieval("Policy", query_text, formatted, limit, latency, emb_latency)
                return formatted
            except Exception as e:
        ...
        results = self._memory_search(query_embedding, "policy", limit, filters=mem_filters if mem_filters else None)
        latency = time.time() - start_time
        self._log_retrieval("Policy (Memory)", query_text, results, limit, latency, emb_latency)
        return results
    ...
    def search_contract_chunks(self, version_id, query_text, limit=5):
        ...
        if self.client:
            try:
                ...
                formatted = [dict(res.payload, score=res.score, id=res.id) for res in results]
                latency = time.time() - start_time
                self._log_retrieval("Contract", query_text, formatted, limit, latency, emb_latency)
                return formatted
            except Exception as e:
                print(f"Qdrant search failed: {e}. Falling back to memory search.")
                
        # Memory Fallback
        results = self._memory_search(query_embedding, "contract", limit, {"version_id": version_id})
        latency = time.time() - start_time
        self._log_retrieval("Contract (Memory)", query_text, results, limit, latency, emb_latency)
        return results
    ...
    def _log_retrieval(self, source, query, results, limit, latency, emb_latency):
        ...
        rag_logger.info(f"Retrieved {len(results)} chunks from {source}", extra={'rag_metrics': metrics})
