...
class Policy(db.Model):
    ...
    )  # GDPR, ISO27001, SOC2, Internal, Vendor, Custom
    ...
    chunks = db.relationship(
        "PolicyChunk", backref="policy", lazy=True, cascade="all, delete-orphan"
    )
...
class PolicyChunk(db.Model):
    ...
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    policy_id = db.Column(
        db.String(36), db.ForeignKey("policies.id", ondelete="CASCADE"), nullable=False
    )
