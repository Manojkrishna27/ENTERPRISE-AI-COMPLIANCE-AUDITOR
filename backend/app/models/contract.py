...
class Contract(db.Model):
    ...
    versions = db.relationship(
        "ContractVersion", backref="contract", lazy=True, cascade="all, delete-orphan"
    )
    reports = db.relationship(
        "Report", backref="contract", lazy=True, cascade="all, delete-orphan"
    )
...
class ContractVersion(db.Model):
    ...
    contract_id = db.Column(
        db.String(36), db.ForeignKey("contracts.id", ondelete="CASCADE"), nullable=False
    )
    version_number = db.Column(db.Integer, nullable=False)
    s3_key = db.Column(db.String(500), nullable=False)
    file_type = db.Column(db.String(10), nullable=False)  # PDF, DOCX
    status = db.Column(
        db.String(30), default="Uploaded"
    ...
    chunks = db.relationship(
        "ContractChunk", backref="version", lazy=True, cascade="all, delete-orphan"
    )
    findings = db.relationship(
        "AIFinding", backref="version", lazy=True, cascade="all, delete-orphan"
    )
...
class ContractChunk(db.Model):
    ...
    version_id = db.Column(
        db.String(36),
        db.ForeignKey("contract_versions.id", ondelete="CASCADE"),
        nullable=False,
    )
