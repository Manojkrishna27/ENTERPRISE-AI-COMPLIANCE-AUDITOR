...
class Settings:
    ...
    DEFAULT_ADMIN_EMAIL: str = os.environ.get(
        "DEFAULT_ADMIN_EMAIL", "admin@contractauditor.com"
    )
    ...
    if not db_uri:
        if is_docker:
            ...
            root_db_path = os.path.abspath(
                os.path.join(
                    os.path.dirname(__file__), "..", "..", "..", "compliance.db"
                )
            )
            db_uri = f"sqlite:///{root_db_path}"
    elif "db:5432" in db_uri and not is_docker:
        root_db_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "..", "..", "compliance.db")
        )
        db_uri = f"sqlite:///{root_db_path}"
    ...
    # Vector DB & Redis
    QDRANT_HOST: str = os.environ.get(
    ...
    USE_LOCAL_STORAGE: bool = (
        os.environ.get("USE_LOCAL_STORAGE", "true").lower() == "true"
    )
    if is_docker:
