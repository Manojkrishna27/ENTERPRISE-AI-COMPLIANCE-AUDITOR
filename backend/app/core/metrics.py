...
def record_http_request(endpoint: str, status_code: int, duration: float):
    ...
    HTTP_REQUESTS_TOTAL["by_endpoint"][endpoint] = HTTP_REQUESTS_TOTAL["by_endpoint"].get(endpoint, 0) + 1
    HTTP_REQUESTS_TOTAL["by_status"][str(status_code)] = HTTP_REQUESTS_TOTAL["by_status"].get(str(status_code), 0) + 1

def record_ai_inference(provider: str, duration: float):
    AI_INFERENCE_LATENCY.append({"provider": provider, "duration": duration, "time": time.time()})

def record_qdrant_query(collection: str, duration: float):
    QDRANT_QUERY_LATENCY.append({"collection": collection, "duration": duration, "time": time.time()})
