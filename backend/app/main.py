...
def init_db_and_seed():
    ...
    print(
        f"[STARTUP] Mode: {'Docker Container' if settings.is_docker else 'Local Environment'}"
    )
    print(
        f"[STARTUP] Email Verification: {'ENABLED' if settings.EMAIL_VERIFICATION_ENABLED else 'DISABLED (Auto-Verify)'}"
    )
    ...
    try:
        ...
        if dept_count == 0:
            default_depts = [
                Department(
                    name="Legal", description="Legal compliance & contract management"
                ),
                Department(
                    name="Compliance",
                    description="Corporate policy & regulatory compliance",
                ),
                Department(
                    name="Procurement", description="Vendor & supplier agreements"
                ),
                Department(
                    name="Engineering", description="Software & technology operations"
                ),
                Department(
                    name="HR", description="Human resources & employment policies"
                ),
                Department(
                    name="Executive", description="Executive leadership & governance"
                ),
            ]
            db_sess.add_all(default_depts)
            db_sess.commit()
            print(
                "[STARTUP] ✅ Departments seeded (Legal, Compliance, Procurement, Engineering, HR, Executive)"
            )
        else:
            print(
                f"[STARTUP] ℹ️ Departments already initialized ({dept_count} departments found)"
            )
        ...
        if not admin_user:
            admin_dept = (
                db_sess.query(Department).filter(Department.name == "Legal").first()
            )
            admin = User(
            ...
            db_sess.commit()
            print(
                f"[STARTUP] ✅ Default System Admin created: {settings.DEFAULT_ADMIN_EMAIL}"
            )
        else:
            print(
                f"[STARTUP] ℹ️ System Admin status: Admin active ({admin_user.email})"
            )
    except Exception as e:
        db_sess.rollback()
    ...
    try:
        ...
        if qdrant_service.is_connected():
            print(
                f"[STARTUP] ✅ Qdrant connected at {qdrant_service.host}:{qdrant_service.port}"
            )
        else:
...
from app.core.metrics import generate_prometheus_metrics, record_http_request
...
@app.get(
    ...
    description="Export system runtime and performance metrics in Prometheus text format",
)
def get_metrics():
