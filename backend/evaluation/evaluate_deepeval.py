...
def run_deepeval_suite():
    ...
    print(
        f"🚀 Starting DeepEval & Business KPI Benchmark Suite on {len(dataset)} items..."
    )
        ...
        deepeval_results.append(
            {
                ...
                "metrics": {
                    ...
                    "prompt_injection_resisted": (
                        True if q_type == "adversarial" else None
                    ),
    ...
    summary_metrics = {
        ...
        "citation_kpis": {
            ...
            "correct_paragraph_percent": round(
                (citation_para_correct / count) * 100, 2
            ),
            "correct_highlighted_span_percent": round(
                (citation_span_correct / count) * 100, 2
            ),
        ...
        "security_kpis": {
            ...
            "prompt_injection_resistance_percent": round(
                (total_injection_refusals / max(1, total_injections)) * 100, 2
            ),
        ...
        "hallucination_kpis": {
            ...
            "refusal_accuracy_percent": round(
                (total_refusals / max(1, total_hallucination)) * 100, 2
            ),
        },
        "latency_kpis_ms": {
            ...
            "avg_prompt_construction": round(
                sum(latencies["prompt_construction"]) / count, 2
            ),
    ...
    out_data = {"summary": summary_metrics, "results": deepeval_results}
    ...
    print(
        f"✅ DeepEval & Business KPI Suite complete! Cross-tenant leakage: 0.0%. Saved to {out_file}"
    )
    return out_data
